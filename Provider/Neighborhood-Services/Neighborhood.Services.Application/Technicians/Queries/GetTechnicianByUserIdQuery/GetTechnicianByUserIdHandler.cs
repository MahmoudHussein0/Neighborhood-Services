...
{
    public class GetTechnicianByUserIdHandler(ITechnicianRepository technicianRepository) : IRequestHandler<GetTechnicianByUserIdQuery, TechnicianDetailsDTO>
    {
        private readonly ITechnicianRepository _technicianRepository = technicianRepository;

        public async Task<TechnicianDetailsDTO> Handle(GetTechnicianByUserIdQuery request, CancellationToken cancellationToken)
        {
            var technician = await _technicianRepository.GetByUserIdAsync(request.ApplicationUserId);
            if (technician == null)
            {
