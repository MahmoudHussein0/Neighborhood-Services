...
{
    public class GetCustomerAddressesByUserIdHandler(ICustomerAddressRepository customerAddressRepository) : IRequestHandler<GetCustomerAddressesByUserIdQuery, List<CustomerAddressDTO>>
    {
        private readonly ICustomerAddressRepository _customerAddressRepository = customerAddressRepository;

        public async Task<List<CustomerAddressDTO>> Handle(GetCustomerAddressesByUserIdQuery request, CancellationToken cancellationToken)
        {
            var customerAddresses = await _customerAddressRepository.GetByUserIdAsync(request.ApplicationUserId);
